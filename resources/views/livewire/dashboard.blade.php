<div x-data="{showScheduleReminder : @entangle('showScheduleReminder')}">
            ...
            <main class="sm:flex">
                ...
                <div class="sm:ml-6">
                    <div class="sm:border-l sm:border-gray-200 sm:pl-6">
                        ...
                        <p class="mt-1 text-base text-gray-500">Please check the URL in the address bar and try again.
                        </p>
                    </div>
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true"
        x-cloak x-show="showScheduleReminder">
        <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0" x-cloak
            x-show="showScheduleReminder">
            <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" aria-hidden="true"
                x-transition-enter="ease-out duration-300" x-transition-enter-start="opacity-0"
                x-transition-enter-end="opacity-100" x-transition-leave="ease-in duration-200"
                x-transition-leave-start="opacity-100" x-transition-leave-end="opacity-0" x-cloak
                x-show="showScheduleReminder"></div>
            ...
            <div class="relative inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full"
                x-cloak x-show="showScheduleReminder" x-transition-enter="ease-out duration-300"
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                ...
                                <p class="text-sm text-gray-500">Did you attend your scheduled vaccination last
                                    {{ Carbon\Carbon::parse($appointment->appointmentDate->date)->format("F d, Y") }}
                                </p>
                                @endif
                ...
                <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
                    @if (isset($appointment))
                    <button type="button" wire:click.prevent='attendedSchedule({{ $appointment->id }})'
                        class="inline-flex justify-center w-full px-4 py-2 text-base font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm">Yes
                        I did!</button>
                    <button type="button" wire:click.prevent='didNotAttendSchedule'
                        class="inline-flex justify-center w-full px-4 py-2 mt-3 text-base font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">No
                        I did not.</button>
                    @endif
                </div>
            </div>
