<div class="py-3 bg-white rounded-lg drop-shadow-xl" x-data="{showSearchModal : false}">
                    ...
                    <div class="grid grid-cols-3 col-span-2">
                        ...
                        <div class="col-span-1 p-2 text-left border-b-4 border-gray-400">
                            <div class="grid items-center w-full grid-cols-3 grid-rows-2">
                                <div class="col-span-1 col-start-1 row-span-1 row-start-1"><p class="text-sm"> <strong class="text-black">Available Slots:</strong> {{ $available_slots }} </p></div>
                                <div class="col-span-1 col-start-2 row-span-1 row-start-1"><p class="text-sm"> <strong class="text-black">Total Slots:</strong> {{ $total_slots }} </p></div>
                                <div class="col-span-1 col-start-1 row-span-1 row-start-2"><p class="text-sm"> <strong class="text-black">Occupied Slots:</strong> {{ $occupied_slots }} </p></div>
                                <div class="col-span-1 col-start-2 row-span-1 row-start-2"><p class="text-sm"> <strong class="text-black">Vaccine:</strong> {{ $vaccine_name }} </p></div>
                                <div class="col-span-1 col-start-3 row-span-2 row-start-1">
                                    <button class="w-auto h-full px-4 py-2 m-2 text-white bg-blue-400 hover:text-blue-800 hover:bg-slate-300 rounded-2xl" type="button" x-on:click="showSearchModal = true">Search All</button>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <table class="min-w-full divide-y divide-gray-300">
                                      <thead>
                                        <tr>
                                          ...
                                          <th scope="col" class="py-3.5 px-3 text-left text-lg font-semibold text-gray-900">Age</th>
                                          <th scope="col" class="py-3.5 px-3 text-left text-lg font-semibold text-gray-900">Purok</th>
                                          <th scope="col" class="py-3.5 px-3 text-left text-lg font-semibold text-gray-900">Sex</th>
                                        ...
                                        @if (count($appointments) > 0)
                                            @foreach ($appointments as $key => $appointment)
                                                <tr class="text-left">
                                                    ...
                                                    <td class="px-3 py-4 text-sm text-gray-500 whitespace-nowrap">{{ Carbon\Carbon::parse($appointment->patient->birthdate)->age }}  years old</td>
                                                    <td class="px-3 py-4 text-sm text-gray-500 whitespace-nowrap">{{ $appointment->patient->purok->name }}</td>
                                                    <td class="px-3 py-4 text-sm text-gray-500 whitespace-nowrap">{{ $appointment->patient->sex }}</td>
                                                    <td class="relative py-4 pl-3 pr-4 text-sm font-medium text-right whitespace-nowrap sm:pr-6 md:pr-0">
                                                    <a href="{{ route('view-patient-information',['patient_id'=>$appointment->patient_id]) }}" class="text-blue-600 hover:text-blue-900">View<span class="sr-only">, {{ $appointment->patient->first_name }} {{ $appointment->patient->middle_name }} {{ $appointment->patient->last_name }}</span></a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        @else
    ...
    {{-- modal start --}}
    <div class="fixed inset-0 z-10 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" x-cloak x-show="showSearchModal">
        <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        
        <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-10 backdrop-blur-md" aria-hidden="true" x-cloak x-show="showSearchModal"  x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"></div>
        ...
        -->
        <div x-cloak x-show="showSearchModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
         class="relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl shadow-blue-800/50 sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
            ...
            <div class="mt-3 text-center sm:mt-5">
                ...
                <div class="mt-2">
                <input type="text" wire:model="searchtermModal" class="block w-full text-indigo-500 border border-blue-300 text-md bg-slate-100 rounded-xl">
                </div>
                ...
                <div class="grid grid-cols-2 p-2 mt-2 bg-slate-100 rounded-xl">
                    ...
                    <div class="container col-span-1 p-2 text-left text-slate-700">
                        <p class="text-md">
                         {{ $appointment_info->patient->first_name }} {{ $appointment_info->patient->middle_name }} {{ $appointment_info->patient->last_name }}
                        </p>
                        <p class="pl-2 text-sm">
                         {{ $appointment_info->patient->sex}} / {{ Carbon\Carbon::parse($appointment_info->patient->birthdate)->age }}  years old
                        </p>
                        <p class="pl-2 text-sm">
                         Purok {{ $appointment_info->patient->purok->name }}
                         </p>
                         <p class="pl-2 text-sm">
                             <a href="{{ route('view-patient-information',['patient_id'=>$appointment_info->patient_id]) }}" class="text-blue-600 hover:text-blue-900">More details >>></a>
                         </p>
                    </div>
                ...
                @else
                    @if (count($appointmentsModal) < 1 && $searchtermModal != "")
                        <div class="flex p-2 mt-2 bg-slate-100 rounded-xl">
                            <span class="py-4 mx-auto text-xl text-slate-400">No patient(s) found with "{{ $searchtermModal }}"</span>
                        </div>
            ...
            <button type="button" x-on:click="showSearchModal = false" class="inline-flex justify-center w-full px-4 py-2 mt-3 text-base font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:col-start-1 sm:text-sm">Close</button>
            </div>
        </div>
        </div>
    </div>
