<div class="py-3 bg-white rounded-lg drop-shadow-xl"
    ...
    <div class="items-center w-full h-full mb-5 space-y-3">
        ...
        <div class="grid grid-cols-7 p-5 mx-10 bg-gray-100 border-collapse rounded-lg shadow-md shadow-indigo-300">
            ...
            @for ($i = 1; $i <= $endDay+$blockSkip; $i++) @if ($i <=$blockSkip) <div
                class="col-span-1 p-5 text-2xl font-bold tracking-wide text-center">
                <span class="m-auto"></span>
            </div>
            @else
            <div wire:click="showconfirmModal({{ $i-$blockSkip }})"
                ...
                <span class="m-auto">{{ $i-$blockSkip }}</span>
            </div>
            @endif
            @endfor
    </div>
    <div class="grid grid-cols-10 grid-rows-4 gap-3 p-5 mx-10 border-collapse rounded-lg">
        ...
        <span class="col-span-1 col-start-10 row-start-2 text-right">-</span>
        <span class="col-span-1 col-start-10 row-start-3 text-right">-</span>
        <span class="col-span-1 col-start-10 row-start-4 text-right">-</span>
    ...
    <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
      ...
      x-transition-leave-end = "opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"  x-cloak x-show="showSuccessModal" class="relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
        ...
        <div class="mt-5 sm:mt-6">
          <button x-on:click="showSuccessModal = false" type="button" class="inline-flex justify-center w-full px-4 py-2 text-base font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:text-sm">Got it!</button>
        </div>
      </div>
    </div>
  </div>
